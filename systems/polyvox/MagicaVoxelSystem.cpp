...
namespace kengine::magica_voxel {
	...
	struct impl {
		...
		static void loadBinaryModel(Entity & e, const char * binaryFile) {
			...
			modelData.free = release(e.id, *em);
		...
		static void unserialize(const char * f, ModelDataComponent::Mesh & meshData, MagicaVoxel::ChunkContent::Size & size) {
			...
			if (!file) {
				kengine_assert_failed(*em, std::string("[MagicaVoxel] Failed to load ") + f);
				return;
			}
		...
		static void readFromStream(T & header, std::istream & s) {
			...
			kengine_assert(*em, s.gcount() == sizeof(header));
		}
		...
		static MeshInfo loadVoxModel(const char * f) {
			...
			if (!idMatches(main.id, "MAIN")) {
				kengine_assert_failed(*em, std::string("[MagicaVoxel] Expected 'MAIN' chunk header in ") + f);
				return MeshInfo{};
			}
			...
			if (!idMatches(first.id, "SIZE")) {
				kengine_assert_failed(*em, std::string("[MagicaVoxel] Expected 'SIZE' chunk header in ") + f);
				return MeshInfo{};
			}
			kengine_assert(*em, first.childrenBytes == 0);
			kengine_assert(*em, first.contentBytes == sizeof(MagicaVoxel::ChunkContent::Size));
			...
			if (!idMatches(voxelsHeader.id, "XYZI")) {
				kengine_assert_failed(*em, std::string("[MagicaVoxel] Expected 'XYZI' chunk header in ") + f);
				return MeshInfo{};
			}
			...
			kengine_assert(*em, voxelsHeader.contentBytes == sizeof(xyzi) + xyzi.numVoxels * sizeof(int));
		...
		static void checkHeader(std::istream & s) {
			...
			kengine_assert(*em, idMatches(header.id, "VOX "));
			kengine_assert(*em, header.versionNumber == 150);
		}
		...
		static void serialize(const char * f, const ModelDataComponent & modelData, const MagicaVoxel::ChunkContent::Size & size) {
			...
			if (!file) {
				kengine_assert_failed(*em, std::string("[MagicaVoxel] Failed to serialize to ") + f);
				return;
			}
...
namespace kengine {
	EntityCreator * MagicaVoxelSystem(EntityManager & em) {
		magica_voxel::impl::em = &em;
		return [](Entity & e) {
			magica_voxel::impl::init(e);
		};
	}
}
