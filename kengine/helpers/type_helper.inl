...
namespace kengine::type_helper {
	...
	entt::entity get_type_entity(entt::registry & r) noexcept {
		...
		if (const auto e = r.view<type_entity_tag<T>>().front(); e != entt::null) {
			if constexpr (putils::reflection::has_class_name<T>())
				kengine_logf(r, very_verbose, "type_helper", "Found existing type entity [%zu] for '%s'", e, putils::reflection::get_class_name<T>());
			return e;
		}
		...
		if constexpr (putils::reflection::has_class_name<T>())
			kengine_logf(r, verbose, "type_helper", "Initializing type entity [%zu] for '%s'", e, putils::reflection::get_class_name<T>());
